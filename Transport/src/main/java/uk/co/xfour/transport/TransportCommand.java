...
/**
 * Handles the /transport command interactions.
 */
public final class TransportCommand implements CommandExecutor, TabCompleter {
    private final JavaPlugin plugin;
    private final List<Module> modules;
    private final TransportItemFactory itemFactory;

    /**
     * Creates a new transport command handler.
     *
     * @param plugin the owning plugin
     * @param modules the configured modules
     */
    public TransportCommand(JavaPlugin plugin, List<Module> modules, TransportItemFactory itemFactory) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if ("reload".equalsIgnoreCase(args[0])) {
            plugin.reloadConfig();
            sender.sendMessage("Transport configuration reloaded.");
            return true;
        }
    ...
    private void sendStatus(CommandSender sender) {
        sender.sendMessage("Transport modules:");
    ...
    private void handleGive(CommandSender sender, String label, String[] args) {
        ...
        TransportItemType type = getItemType(args[itemIndex]);
        if (type == null) {
            sender.sendMessage("Unknown transport item. Available: " + String.join(", ", getItemKeys()));
            return;
        }
        int amount = 1;
    ...
    private TransportItemType getItemType(String key) {
